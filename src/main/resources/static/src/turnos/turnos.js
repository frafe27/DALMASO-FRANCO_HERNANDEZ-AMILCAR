            ...
            (turno) => `
                      <tr>
                          <td>
                              ...
                              Fecha de Ingreso: ${
                                turno.pacienteSalidaDto.fechaIngreso
                              }<br>
                              Domicilio: ${formatDomicilio(
                                turno.pacienteSalidaDto.domicilioSalidaDto
                              )}
                          </td>
                          <td>
                              ...
                              Número de Matrícula: ${
                                turno.odontologoSalidaDto.numeroMatricula
                              }
                          </td>
                          <td>${new Date(
                            turno.fechaYHora
                          ...
                          <td class="text-center">
                              <button data-id="${
                                turno.id
                              }" class="text-red-500 dltTurno"><i class="fas fa-trash-alt"></i></button>
                              <button data-id="${
                                turno.id
                              }" class="text-red-500 editTurno"><i class="fas fa-edit"></i></button>
                          </td>
                      </tr>
                  `
          )
    ...
    fetch(`http://localhost:8080/turnos/eliminar/${id}`, {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          throw new Error(
            "Error al eliminar el turno. Código de error: " + response.status
          );
        }
      })
